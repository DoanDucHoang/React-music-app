...
function Card({ item, title }) {
  ...
  const styleImage = title => {
    switch (title) {
      case 'Gần Đây':
        return 'relative lg:h-36 md:h-36 w-full overflow-hidden object-cover object-center transform group-hover:scale-125 duration-1000';
      case 'Mới Phát Hành':
        return 'relative lg:h-14 md:h-14 w-full overflow-hidden object-cover object-center duration-1000';
      case 'Album':
        return 'relative lg:h-14 md:h-14 w-full overflow-hidden object-cover object-center duration-100';
      default:
        return 'relative lg:h-full md:h-48 w-full overflow-hidden object-cover object-center transform group-hover:scale-125 duration-1000';
    }
  };
  ...
  return (
    <div className={`${styleCard2(title)} z-10`} key={item.id}>
      <div className={`${styleCard(title)}`}>
        ...
        >
          ...
          >
            {title === 'Mới Phát Hành' || title === 'Album' ? (
              ''
            ) : (
              <AiOutlineHeart />
            )}
