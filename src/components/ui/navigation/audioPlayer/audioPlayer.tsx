...
import { setControls, setPlaying, setCurrent } from '../../../../stores/player';
...
const AudioPlayer = () => {
  ...
  const { current, isNextSong, theme } = useSelector(
  ...
  const handleChangeMusic = (callback: Function) => {
    resetRepeatState();
    callback();
  };
  ...
  return (
    <div
      className={`flex px-4 justify-between items-center h-full ${
        theme ===
        'https://zmp3-static.zmdcdn.me/skins/zmp3-v6.1/images/theme-background/zma.svg'
          ? 'bg-[url("https://zmp3-static.zmdcdn.me/skins/zmp3-v6.1/images/theme-player/zma.png")]'
          : theme ===
    ...
    >
      <div className="min-w-[11.25] w-[30%] text-white">
        {current && (
      ...
      <div className="max-w-[45rem] w-[35%] flex flex-col items-center text-white">
        <div className="flex items-center gap-x-4">
          ...
          <button
            onClick={() => handleChangeMusic(updateCurrentPre)}
            className="w-9 h-9 flex items-center justify-center hover:text-pink-400"
          >
            <Icon size={16} name="playerPrev" />
          </button>
          <button
            onClick={controls[state?.playing ? 'pause' : 'play']}
            className="w-9 h-9 flex border-2 rounded-full items-center justify-center hover:border-pink-400 hover:text-pink-400"
          >
            <Icon size={16} name={state?.playing ? 'pause' : 'play'} />
          </button>
          <button
            onClick={() => handleChangeMusic(updateCurrentNext)}
            className="w-9 h-9 flex items-center justify-center hover:text-pink-400"
          >
        ...
        <div className="w-full flex items-center gap-x-2 mt-1.5 text-white">
          {audio}
